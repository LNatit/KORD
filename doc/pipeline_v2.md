...
#### 🟢 第 5 层：意图合并层 (Intent Alias Layer)
*   **动作**：对比双方的 `intent`。
*   **逻辑**：只要 `intent` 相同（且不为 `unknown`）：
    *   **完全无视 `hijack_mode` 的值！** 因为无论谁吃谁，意图都达成了。
    *   根据动作的危险程度直接放行。例如：移动类意图直接 `SAFE`，战斗/系统类意图可能给个 `INFO` 或 `WARNING`（提示玩家：“这两个相同功能的键绑在一起了，目前某模组接管了该功能”）。
*   **结果**：短路返回，判定结束。

    ...
    protected void apply(Map<Identifier, ContextDefinition> map, ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        for (Map.Entry<Identifier, ContextDefinition> entry : map.entrySet()) {
            ...
            if (definition.isInvalid()) {
                Kord.LOGGER.debug("Context definition in '{}' is invalid and will be ignored.", id);
                continue;
            }
            ...
            try {
                ...
                if (existing != null) {
                    Kord.LOGGER.warn("Context id '{}' from '{}' overrides an existing context.", definition.id(), id);
                }
                KeyContext.register(definition.toKeyContext());
                loaded++;
            } catch (Exception e) {
                Kord.LOGGER.warn("Failed to register context '{}' from '{}': {}", definition.id(), id, e.getMessage());
            }
        }

        Kord.LOGGER.info("Loaded {} context definitions.", loaded);
        profiler.pop();
    }
}
